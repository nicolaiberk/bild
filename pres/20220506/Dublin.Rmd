...
Presentation prepared for the 4th Annual CompText Conference, Dublin
...
knitr::include_graphics("collage_ukraine.png")
...
2. **Predispositions** structure individual susceptibility.
...
- Experimental research suggests that **holding a consideration in mind decreases susceptibility to framing** `r Citep(myBib[c("Druckman2012")])`.
...
#### Problem: migration content is rare

### Solution: oversample migration content

1. Define **seed dictionary** of migration terms.
2. Extend dictionary using word embeddings.
3. Oversample from articles mentioning immigration terms.



---

# Identifying migration content II

<br>

- RA annotates 1800 articles.
- Fine-tune **German BERT** to identify migration content (accuracy 0.96, F1: 0.94, recall: 0.93, precision: 0.95).
- Predict label for 2.5M news articles: 90,000 about migration.

---

# Identifying Considerations about Immigration `r Citep(myBib["Nicholson2020"], before = "following ")`

<br>

- Apply **STM** with 60 topics `r Citep(myBib["Roberts2014"])` to identify migration frames.
- **Annotate** all topics, **select** the ones about crime.
- Estimate daily prevalence for each newspaper.
...
trendplot <- 
  ...
  theme_xaringan() + 
  scale_color_discrete(name = "Outlet", labels = c("Bild", "Other"))
...
## Measurement: News Consumption and Availability

#### Exposure to Treatment

- Question on newspapers consumed last week.
- Simple binary variable indicating *Bild* consumption.
- Alternative: Calculate individual exposure to crime framing.

--

#### Availability

- **Initial migration opinion**.
- Reporting **migrant crime as MIP**.
...
## with resampling
## still need to think about implications of this
gles_p_long <-
  gles_p_long %>%
  ...
  group_by(lfdn) %>%
  slice(1) %>%
  mutate(init_mig_res = `1130_clean`) %>%
  select(lfdn, init_mig_res) %>%
  right_join(gles_p_long, by = "lfdn")
...
gles_p_long <- 
  ...
  group_by(lfdn) %>% 
  slice(1) %>% 
  mutate(treat_alt = ifelse(is.na(`1661a_clean`), NA, `1661a_clean` > 0)) %>% 
  select(treat_alt, lfdn) %>% 
  right_join(gles_p_long, by = "lfdn")

## vis dv by treatment group and conditioning var

did_desc_plot <- 
  gles_p_long %>%
  filter(date_clean < as.Date("2018-01-01")) %>% 
  filter(!is.na(`1130_clean`) & !is.na(bild_init)) %>% 
  mutate(init_mig_cat = 
           case_when(
             init_mig > 0 ~ "01 - Conservative",
             init_mig == 0 ~ "02 - Neutral",
             init_mig < 0 ~ "03 - Liberal"
           )) %>% 
  group_by(bild_init, wave, init_mig) %>% 
  select(`1130_clean`, date_clean) %>% 
  summarise(dv = mean(`1130_clean`, na.rm = T),
            dv_sd = sd(`1130_clean`, na.rm = T),
            date_min = min(date_clean, na.rm = T),
            n_resps = n()) %>%
  mutate(
    lower = dv + qt(0.025, n_resps - 1) * (dv_sd/sqrt(n_resps)),
    upper = dv + qt(0.975, n_resps - 1) * (dv_sd/sqrt(n_resps))
  ) %>% 
  ggplot(aes(date_min, dv, ymin = lower, ymax = upper, col = !bild_init, fill = !bild_init)) +
  geom_line() +
  geom_ribbon(alpha = 0.5) +
  geom_vline(xintercept = as.Date("2017-02-01"), lty = 2, col = "red") +
  facet_grid(~init_mig) +
  ggtitle("Average migration opinion dependent on initial migration opinion", 
          "Bild vs. other major daily newspapers, 2016-2017") + 
  xlab("Date") + ylab("Migration Opinion") +
  theme_minimal() +
  theme(legend.position = "none")

ggsave("C:/Users/nicol/Dropbox/PhD/Papers/Bild/github/bild/paper/vis/did_descriptive.svg",
       did_desc_plot,
...
did_groups <- 
  ...
  geom_col() + 
  theme_xaringan() + xlab("") + ylab("") +
  ggtitle("Effect across initial migration attitude")
...
ggsave(plot = did_bootstrap,
       filename = 'C:/Users/nicol/Dropbox/PhD/Papers/Bild/github/bild/paper/vis/DiDXinit_imm.svg', device = "svg")
...
plot_did_inter <- 
  ests %>% 
  filter(row.names(.) %in% c("postTRUE:treatTRUE", "postTRUE:treatTRUE:mip_mig_crime_initTRUE")) %>% 
  mutate(name = fct_reorder(row.names(.), desc(row.names(.)))) %>% 
  ggplot(aes(y = name, x = points, xmin = `2.5 %`, xmax = `97.5 %`)) +
  geom_pointrange(size = 2) +
  geom_vline(xintercept = 0, col = "red", lty = 2, size = 2) +
...
- Effect conditioned by **Prior availability** of consideration.
...
- How important are these effects in **more fragmented media landscapes**?
