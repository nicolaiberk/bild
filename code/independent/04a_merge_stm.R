...
merged_data <- 
  merged_data %>% 
  ## wide -> long
  select(lfdn:`1220_lag`, n_mig_sal_7d_bild:n_mig_fra_7d_sz, -contains("2880"), days_read) %>% 
  pivot_longer(cols = n_mig_sal_7d_bild:n_mig_fra_7d_sz, 
...
merged_data <- 
  ...
  summarise(across(c(n_mig_sal:n_mig_fra), sum))  

merged_data <- 
  merged_data %>% 
  # divide by number of migration articles to get share
  mutate(across(n_mig_sal:crime_prob, ~.x/n_mig_fra, .names = "{.col}_share"))
...
merged_data %>%  
  fwrite(., file = here(paste0("data/processed/merged_stm.csv")))
