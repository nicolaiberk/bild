...
prevalence_daily <- fread(here("data/processed/bert_crime_daily.csv"))
...
prevalence_daily <- 
  ...
  group_by(paper) %>% 
  mutate(
    across(crime_label:n_articles, ~roll_sum(.x,    7, fill=NA, align = 'right', na.rm = T), .names = "{.col}_7d")
...
merged_data <- 
  merged_data %>% 
  mutate(across(crime_label_7d_Bild:n_articles_7d_SZ, ~ifelse(is.na(.x), 0, .x))) %>% 
  mutate(
...
merged_data <- 
  merged_data %>% 
  ## wide -> long
  select(lfdn:`1220_lag`, crime_label_7d_Bild:n_articles_7d_SZ, -contains("2880"), days_read) %>% 
  pivot_longer(cols = crime_label_7d_Bild:n_articles_7d_SZ, 
...
merged_data <- 
  merged_data %>% 
  pivot_longer(cols = crime_label_7d:n_articles_7d, 
...
merged_data <- 
  ...
  mutate(across(crime_label:n_articles,
                ~case_when(
  ...
  summarise(across(c(crime_label:n_articles), sum))  

merged_data <- 
  merged_data %>% 
  # divide by number of migration articles to get share
  mutate(across(crime_label:crime_prob, ~.x/n_articles, .names = "{.col}_share"))
...
merged_data %>%  
  fwrite(., file = here(paste0("data/processed/merged_bert.csv")))