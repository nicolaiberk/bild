...
knitr::opts_chunk$set(echo = F, message = F, warning = F, cache = T, fig.height=6, fig.width=10, fig.align='center', eval = T, fig.pos = "!h", out.extra = "")
...
To provide a baseline expectation regarding the effect from experimental work, it is useful to consider the expected effect size at this point. Despite the abundance of framing effects studies, there is no common standard to assess the magnitude of framing effects. Even the correct measurement of the magnitude is a contested question, as different control groups can be compared [@Druckman2001; @Chong2007a, 109f for a discussion]. Additionally, framing effects differ based on the polarization and salience of the issue [@Lecheler2009b], repeated exposure [@Chong2007competitiveFraming], the sender [@Slothuus2010], political knowledge [@Chong2007b], as well as the reference of certain groups [@Nelson1996]. Generally, reported effect size around 30% of the scale are not uncommon [@Slothuus2010], sometimes even over 40% [@Chong2007a, 104], but most results lie in between 10% and 25% of the scale when comparing opposing frames pushing respondents in opposite directions\footnote{As many studies do not include a control group, this is the most comparable metric.} [@Lecheler2009b; @Nelson1997Psychology; @Nelson1997Rally; @Slothuus2008; @Slothuus2010]. [add reference to GSS running welfare panel with welfare vs assistance to the poor form \url{https://www.youtube.com/watch?v=a_CmfRjZ_oY}. Problem with this: much better data to show your argument, kinda opposite result - look into this though, maybe after finishing the paper!]
...
[simplify all of these sections, follow economist papers from course]

According to the expectancy-value model (see section \@ref{framing-effects}), the most direct implication of increased exposure to crime frames in migration coverage should be a more conservative assessment of immigration. Figure \@ref(fig:singleplot) visualizes the effect of the change in framing on immigration attitudes, with higher values indicating that *Bild* readers change their attitudes in a more restrictive direction, compared to the control group. Dashed vertical lines indicate the expected effect from the experimental literature. The dotted lines indicate the equivalence boundaries. Surprisingly, the model suggests a clear null effect. This effect is not significantly different from zero, but significantly smaller than the above-defined minimal effect of 0.15 on the seven-point Likert scale. The point estimate indicates a `r round(ATT, 3)`-point change among *Bild* readers following treatment (*t =* `r round(t_ATT, 2)`, *p =* `r round(p_ATT, 2)`). This represents only a fraction (`r round(ATT/0.6, 3)*100`%) of the already conservative experimental estimate of 0.6.
...
Following [@Nelson1996], table \@ref(tab:cortable) also regresses immigration attitudes on attitudes towards crime and investigates whether this association changes as a result of the change in coverage (note that all constituent terms are included but only those regarding the association of crime and immigration attitudes reported). Similar to the differences between the groups, I find a weaker association among readers of the tabloid, but no significant or substantively large difference post-treatment or when interacted with the DiD-term. Respondents do not seem to associate crime more strongly with migration following the change in migration coverage in *Bild*.
